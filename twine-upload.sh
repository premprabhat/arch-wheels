...
if [[ ${TESTPYPI_UPLOAD} == true  || ${PYPI_UPLOAD} == true ]]; then
    ...
    if [[ ${TESTPYPI_UPLOAD} == true ]]; then
        twine upload --skip-existing --repository-url https://test.pypi.org/legacy/ -u $TESTPYPI_USERNAME -p $TESTPYPI_PASSWORD ${TRAVIS_BUILD_DIR}/wheelhouse/*.tar.gz
    ...
    if [[ ${PYPI_UPLOAD} == true ]]; then
        twine upload --skip-existing -u $PYPI_USERNAME -p $PYPI_PASSWORD ${TRAVIS_BUILD_DIR}/wheelhouse/*.tar.gz
